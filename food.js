const foodPlaces = [
  ...
  document.getElementById("searchButton").addEventListener("click", () => {
    const selectedCuisine = document.getElementById("cuisineDropdown").value; // Get selected value
    ...
    if (!selectedCuisine) {
      output.innerHTML = "<p>Please select a cuisine from the dropdown menu.</p>";
      return;
    }
  
    // Filter and rank the data
    const filteredPlaces = foodPlaces
      .filter(place => place.cuisine === selectedCuisine) // Match exact cuisine
      .sort((a, b) => b.rating - a.rating || b.reviewsCount - a.reviewsCount); // Sort by rating, then reviews count
  
    // Handle no results
    if (filteredPlaces.length === 0) {
      output.innerHTML = `<p>No food places found for the cuisine "${selectedCuisine}". Try another cuisine!</p>`;
      return;
    }
